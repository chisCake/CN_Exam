...
namespace CN_Exam
{
	...
	{
		// Адреса-исключения
		static readonly List<IPAddress> forbiddenIPs = new List<IPAddress>()
		{
			new IPAddress("10.10.01.01"),
			new IPAddress("172.16.196.200"),
			new IPAddress("192.168.100.100")
		};
		...
		public static void Test()
		{
			while (true)
			{
				try
				{
					...
					var ip = new IPAddress(ipStr);
					if (forbiddenIPs.Any(forbidden => forbidden.ToString() == ip.ToString()))
						throw new Exception("Этот ip невозможно использовать");

					Console.Write("Введите маску сети: ");
					string netmask = Console.ReadLine();

					var network = GetNetwork(new IPAddress(ipStr), new Netmask(netmask));
					var host = GetHost(new IPAddress(ipStr), new Netmask(netmask));
